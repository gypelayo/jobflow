# Documentation

## Website (GitHub Pages)
Deployed at: https://gypelayo.github.io/jobflow

- [Landing Page](./index.html)
- [Install Guide](./install.html)
- [Privacy Policy](./privacy-policy.html)

## Project docs
- [README](../README.md) — overview and quick start
- [ROADMAP](../ROADMAP.md) — shipped, active, backlog, pricing
- [PROJECT_MAP](../PROJECT_MAP.md) — full architecture and source map
- [CONTRIBUTING](../CONTRIBUTING.md) — dev setup and conventions
- [Testing Guide](../extension/dashboard/TESTING.md)
- [Release Process](../.github/RELEASE_PROCESS.md)
